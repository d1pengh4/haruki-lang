...
**Haruki** is a real-time AI-powered Korean Sign Language (KSL) translator. It uses Google MediaPipe Holistic for landmark extraction, then matches gestures against a Supabase database using cosine similarity and Dynamic Time Warping (DTW).
...
**Data flow:**
1. `WebcamCapture` loads MediaPipe Holistic from CDN, captures landmarks per frame
2. `featureExtraction.ts` converts raw landmarks → 33D invariant feature vectors (position/scale/rotation invariant)
3. `RecognitionMode` maintains a 90-frame (~3s) sliding window buffer, compares against all signs in Supabase via DTW + cosine similarity
...
| File | Purpose |
|------|---------|
| `src/lib/featureExtraction.ts` | **Core ML logic** — 33D hand feature extraction, DTW, cosine similarity |
...
- `landmarks_sequence` (JSONB) — array of frames; each frame contains raw pose/hand/face landmarks + pre-calculated 33D feature vectors
...
- **ML**: Google MediaPipe Holistic (loaded via CDN), custom DTW/cosine matching
...
- Always work with 33D feature vectors — never raw landmark coordinates — for matching
...
- **MediaPipe loads from CDN**: `WebcamCapture` loads MediaPipe Holistic via `<script>` tags at runtime. It won't work in offline environments or if the CDN URLs are blocked by CSP.
